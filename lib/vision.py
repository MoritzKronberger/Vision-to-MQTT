...
def read_output(filename: str) -> tuple[Garbage | None, Boat] | None:
    try:
        with open(filename, 'r') as f:
            ...
            distance_boat_garbage = distance_to_origin(
                garbage.position_x_m, garbage.position_y_m)
            angle_boat_garbage_deg = angle_x_axis_to_point_deg(
                garbage.position_x_m, garbage.position_y_m)
            boat = Boat(
                boat_visible=True,
...
def distance_to_origin(x: float, y: float, x_offset: float = 0, y_offset: float = 0) -> float:
...
def angle_x_axis_to_point_deg(x: float, y: float, x_offset: float = 0, y_offset: float = 0) -> float:
